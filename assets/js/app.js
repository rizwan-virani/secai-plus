/* ============================================================================
   ...
   Architecture
   ------------
   - contentData.js loads first and defines the global `SECAI` namespace with
...
(function () {
  ...
  var S = window.SECAI || {};
  /* ---- Exam figures: single source of truth is SECAI.exam (contentData.js).
  ...
  var Store = (function () {
    ...
    return {
      ...
      get: function (k, fb) { return read("secai." + k, fb); },
      set: function (k, v) { write("secai." + k, v); },
      del: function (k) { try { localStorage.removeItem("secai." + k); } catch (e) {} }
    };
  })();
  ...
  /* ===========================================================================
     ...
     =========================================================================== */
  function certVer() { var v = window.SECAI_VER || ""; return v ? "?v=" + v : ""; }
  ...
  /* Lazily fetch a domain's reading module the first time it's opened, so the
     ...
     script populates SECAI.reading[id]; once cached it renders instantly. */
  ...
  function loadDomainModule(id, onReady, onFail) {
    ...
    s.src = "assets/js/content/domain" + id + ".js" + (window.SECAI_VER ? "?v=" + window.SECAI_VER : "");
    s.async = true;
    s.onload = function () {
  ...
  /* ===========================================================================
     BOOT
     =========================================================================== */
  /* Fill the hero exam-details line from SECAI.exam so the four headline figures
